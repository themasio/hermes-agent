...
Environment variables:
    ...
    MATRIX_FREE_ROOM_BOT_BLOCKLIST
                              Comma-separated MXIDs that do NOT get the free-room
                              mention bypass — anti-loop for peer Hermes-bot
                              identities in shared free rooms (alias of
                              matrix.free_room_bot_blocklist)
...
def _apply_yaml_config(yaml_cfg: dict, matrix_cfg: dict) -> dict | None:
    ...
    # free_room_bot_blocklist: in free rooms, these MXIDs do NOT bypass
    # require_mention (still need an @mention to engage). Anti-loop for
    # multi-Hermes-profile rooms where peer-bots post but should not
    # trigger ambient handlers.
